...
function mergeSmallGroups(groups, tabs, embeddings, opts) {
...
export function detectExcursionsTargeted(tabs, embeddings, opts = {}) {
  ...
  console.log(
    `[cluster] detectExcursionsTargeted: tabs=${tabs.length} target=${targetAvgSize} window=${window} penalty=${sizePenalty.toFixed(3)} smallPenalty=${smallSizePenalty.toFixed(3)}`,
  );
...
export function clusterByEmbeddingsTargeted(tabs, embeddings, opts = {}) {
  ...
  const desiredK = Math.max(1, Math.round(tabs.length / targetAvgSize));
  console.log(
    `[cluster-agg] start: tabs=${tabs.length} targetAvg=${targetAvgSize} → desiredK=${desiredK} penalty=${sizePenalty.toFixed(3)} smallPenalty=${smallSizePenalty.toFixed(3)}`,
  );

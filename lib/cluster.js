...
export function absorbSingletons(groups, tabs, embeddings, lockedTabIds = new Set()) {
  ...
  let absorbed = 0;
  while (working.length >= 2) {
    const i = working.findIndex((g) => g.length === 1 && !isLocked(g));
    if (i === -1) break;
    ...
    const candidates = openSingletons.length ? openSingletons : (open.length ? open : neighbours);
    let target = -1;
    ...
    console.assert(target !== -1, "a singleton always has a strip neighbour when groups.length >= 2");
  ...
  console.assert(working.every((g) => g.length >= 2) || working.length === 1, "singleton survived absorption");
  return working;
}
...
function withIdenticalCohorts(tabs, embeddings, opts, clusterCore) {
  ...
  if (!groupIdentical && !ownGroup) return finish(clusterCore(tabs, embeddings));

  let cohorts = findIdenticalCohorts(tabs);
  if (opts.contiguousOnly) {
    const posById = new Map([...tabs].sort((a, b) => a.index - b.index).map((t, i) => [t.id, i]));
    cohorts = cohorts.flatMap((c) => splitIntoStripRuns(c, posById));
  }
  if (!cohorts.length) return finish(clusterCore(tabs, embeddings));

  const held = new Set(cohorts.flat().map((t) => t.id));
  ...
  debugLog(`[cluster] identical cohorts: ${cohorts.length} holding ${held.size} tabs, ${rest.length} tabs sized normally`);
  ...
  if (ownGroup || !result.groups.length) {
    result.groups = sortGroupsByStrip([...result.groups, ...cohorts]);
    return finish(result, ownGroup ? held : new Set());
  } else {
    const embByTabId = new Map(tabs.map((t, i) => [t.id, embeddings[i]]));
    ...
    result.groups = sortGroupsByStrip(working);
  }

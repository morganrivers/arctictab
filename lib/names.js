...
function getVocab() {
  if (!vocabPromise) {
    vocabPromise = (async () => {
      debugLog(`[names] embedding vocabulary: ${CURATED.length} curated + ${HEAD_NOUNS.length} heads`);
      const curEmb = await embedNames(CURATED);
      const headEmb = await embedNames(HEAD_NOUNS);
      debugLog("[names] vocabulary ready");
      return {
...
function assignName(ctx, vocab, style, used, cfg) {
  ...
  if (group.every(isNewTab)) {
    ...
    debugLog(`[names] "${name}" via new-tab special-case (size ${group.length})`);
    return name;
  }
  ...
  const take = (name, tier) => {
    ...
    debugLog(`[names] "${name}" via ${tier} (size ${group.length})`);
    return name;
  };

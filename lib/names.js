...
// Tunable thresholds. Cosine values are for Snowflake/snowflake-arctic-embed-xs
// (mean-pooled, normalized). Each group's chosen tier + sims are logged so
// these can be calibrated against real tab sets.
const HEAD_SIM_FLOOR = 0.22;     // min centroid<->category-noun cosine to compose
const CURATED_SIM_FLOOR = 0.27;  // min centroid<->curated-name cosine to use it
const FRAC_FLOOR = 0.34;         // keyword must appear in >=34% of the group's tabs
...
const HEAD_NOUNS = [
  ...
  "Jobs", "Quotes", "Plans", "Goals", "Logs", "Feed", "Wiki", "Forum",
...
function pickKeyword(ranked, N) {
  for (const r of ranked) {
    if (r.frac < FRAC_FLOOR) continue;
    const distinct = N <= 1 || r.df <= Math.max(1, Math.floor(N * 0.5));
...
function assignName(ctx, vocab, style, used) {
  const { group, ranked, centroid, N } = ctx;
  ...
  const kw = style === "one" ? null : pickKeyword(ranked, N);

  // 1. Compose <distinctive keyword> + <nearest category noun>.
  if (kw) {
    for (const h of heads) {
      if (h.sim < HEAD_SIM_FLOOR) break;
  ...
  // 2. Best curated semantic match honoring the style filter.
  for (const c of curated) {
    if (c.sim < CURATED_SIM_FLOOR) break;
...
export async function nameGroups(groups, ctx, opts = {}) {
  ...
  for (const i of order) {
    names[i] = assignName(
      { group: groups[i], ranked: ranked[i], centroid: centroids[i], N: groups.length },
      vocab,
      style,
      used,
    );

...
export function buildBm25(docs) {
  ...
  return { postings, terms, docLengths, avgdl, docCount: docs.length };
}
...
export function prefixTerms(index, prefix) {
  console.assert(Array.isArray(index.terms), "index must carry a sorted term list");
  console.assert(prefix, "prefix must be non-empty");
  const out = [];
  for (let i = lowerBound(index.terms, prefix); i < index.terms.length; i++) {
    if (!index.terms[i].startsWith(prefix)) break;
    out.push(index.terms[i]);
  }
  return out;
}

export function queryTermWeights(index, queryTokens) {
  const weights = new Map();
  const last = queryTokens.length - 1;
  for (let i = 0; i < queryTokens.length; i++) {
    weights.set(queryTokens[i], 1);
    if (i !== last) continue;
    for (const term of prefixTerms(index, queryTokens[i])) {
      if (!weights.has(term)) weights.set(term, PREFIX_WEIGHT);
    }
  }
  return weights;
}
...
export function scoreBm25(index, queryTokens) {
  ...
  if (!N) return scores;
  for (const [term, weight] of queryTermWeights(index, queryTokens)) {
    ...
    for (const [d, freq] of list) {
      ...
      scores[d] += weight * idf * ((freq * (BM25_K1 + 1)) / denom);
    }
  }
  return scores;
}
...
export function rankTabs({ bm25Index, texts, embeddings, query, queryEmbedding, weights = { bm25: 0.7, embed: 0.3 }, limit = SEARCH_LIMIT }) {

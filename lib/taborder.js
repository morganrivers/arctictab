...
// without relocating any content tab. A cluster only becomes a Firefox group when
// its groupable members form a contiguous run among the groupable tabs; otherwise
// grouping them would force Firefox to relocate a content tab, so it is skipped
// (only the explicit Re-organize action is allowed to reorder content tabs).
// Non-groupable tabs (about:newtab, about:debugging, ...) sitting between members
// don't block grouping — group() shoves them aside. Returns the group runs to draw
// and the tab ids to ungroup because they left their cluster.
export function planGroupSync(liveTabs, groups) {
  // Contiguity is measured over groupable tabs only. A non-groupable tab
  // (about:newtab, about:debugging, ...) wedged between a cluster's members is
  // not a gap: Firefox's group() pushes those junk tabs to the group boundary.
  // A cluster is grouped only when no OTHER groupable tab sits between its
  // members, so forming the group never relocates a content tab.
  const posById = new Map(
    [...liveTabs].filter(isGroupable).sort(byIndex).map((t, i) => [t.id, i]),
  );
  const live = new Set(liveTabs.map((t) => t.id));
  const keep = new Set();
  const group = [];
  for (const g of groups) {
    const members = g.filter(isGroupable).map((t) => t.id).filter((id) => live.has(id));
